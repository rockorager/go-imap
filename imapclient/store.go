...
func (c *Client) store(uid bool, seqSet imap.SeqSet, store imap.StoreData, options *imap.StoreOptions) *FetchCommand {
	...
	case *imap.StoreXGmLabels:
		...
		case imap.StoreFlagsDel:
			enc.Special('-')
		default:
			panic(fmt.Errorf("imapclient: unknown store op: %v", store.Op))
		}
		enc.Atom("X-GM-LABELS")
		if store.Silent {
			enc.Atom(".SILENT")
		}
		enc.SP().List(len(store.XGmLabels), func(i int) {
			enc.String(store.XGmLabels[i])
		})
	}
	enc.end()
	return cmd
...
func (c *Client) Store(seqSet imap.SeqSet, store imap.StoreData, options *imap.StoreOptions) *FetchCommand {
...
//
// See Store.
func (c *Client) UIDStore(seqSet imap.SeqSet, store imap.StoreData, options *imap.StoreOptions) *FetchCommand {
